...
#                     self.f_history.append(self.tokenizer.encode(subtext))
...
#         # print(self.tokenizer.decode(list(chain(*self.history))))
#         # print(self.tokenizer.decode(list(chain(*self.f_history))))
#         instance = build_input_from_segments(self.persona, self.history, partial_out_ids,
#                                              self.tokenizer, with_eos=False, f_history=self.f_history)
...
class TransformerAgent(Agent):
    ...
    def act(self):
        ...
        if self.args.eval_type == "hits@1" and len(self.candidates) > 0:
            instances = defaultdict(list)
            for candidate, _ in self.candidates:
                instance = build_input_from_segments(self.persona, self.history[-(2*2+1):], candidate, self.tokenizer, f_history = self.history[:-(2*2+1)] )
                for input_name, input_array in instance.items():
                    instances[input_name].append(input_array)
