...
def output_gen(question):
    ...
    for data_batch_idx, data_batch in enumerate(dataset_iter):
        ...
        #        answer[i][torch.sort(torch.max(subscores, 1)[0], descending=True)[1][0:min(2, len(FindOidx))]] = idxI
        pred_list.append(answer)
    #dete_result, question_list = entity_predict(dataset_iter=test_iter) # required output 
    del model
...
# In[44]:
    if is_empty == 0:
        return 'I dont know bro'
    else:
        return matches[0] # PRINTS THE OUTPUT
...
def query():
    ...
    return 'data:' + output_gen(question)
    #return jsonify({'data': output_gen(question)})
# driver function 
